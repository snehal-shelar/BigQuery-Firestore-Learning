...
# Initialize the BigQuery client once for better performance
...
def gcs_to_bigquery_pipeline(cloud_event):
    ...
    load_job = bq_client.load_table_from_uri(uri, staging_table, job_config=load_config)
    ...
    query_job = bq_client.query(transform_sql)
    query_job.result()  # Wait for the transformation to complete
